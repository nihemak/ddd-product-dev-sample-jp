...
* **アクティビティ**:
    ...
    4. 必要に応じて、[`docs/domain/ubiquitous-language.md`](./domain/ubiquitous-language.md) や [`docs/domain/domain-model.md`](./domain/domain-model.md) の見直しも行います（特に新たな概念の発見や既存概念の変更があった場合）。
...
* **`openapi-typescript-codegen` (フロントエンド)**: バックエンドの OpenAPI スキーマ (`backend/docs/openapi.json` 等) から、TypeScript の型定義と API クライアントコードを自動生成します。これにより、フロントエンドとバックエンド間のインターフェースの整合性を保ち、型安全な API 呼び出しを容易にします。生成されたコードは `frontend/src/lib/api/generated/` に配置されます。
* **`React Query (TanStack Query)` (フロントエンド)**: サーバー状態の管理（データのフェッチ、キャッシュ、同期、更新）を効率的に行うためのライブラリです。ローディング状態やエラー処理を簡素化し、パフォーマンスの高いデータ取得ロジックをカスタムフックとしてカプセル化するのに役立ちます。
