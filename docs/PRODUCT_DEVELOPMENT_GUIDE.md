...
*   **ユビキタス言語の徹底**: **プロダクト定義から実装、テストに至るまで**、会話する際、コードを書く前、ドキュメントを作成する際に、常に [`docs/domain/ubiquitous-language.md`](./domain/ubiquitous-language.md) を参照・意識します。レビューでは用語の不一致を指摘します。
*   **コードとドキュメントの同期**: コード変更（特にドメイン層やインターフェース）と同時に、関連するドキュメント（プロダクト定義、要求定義、**ユビキタス言語、ドメインモデル**など）も更新します。Pull Request にドキュメント変更も含めることを原則とします。
*   **軽量ドキュメント**: PlantUML/Mermaid や ADR など、テキストベースでバージョン管理しやすく、メンテナンスコストの低い形式を優先します。
*   **レビューの活用**: コードレビュー時に、関連ドキュメント（プロダクト定義含む）との整合性、**特にユビキタス言語とドメインモデルとの一貫性**を確認します。
